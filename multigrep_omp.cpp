...
int main(int argc, char *argv[]) {
    ...
    if (parall_file) {
        ...
        for (unsigned int j = 0; j < files.size(); j++) {
            ifstream f((files[j]).c_str());
            if (!f.is_open())
                continue; /* nonexistant file */
            int k = 0;
            string tline;
            while (mygetline(&f, &tline, &term) == 0) { /* while able to read in line */
                k++; /* incr line counter */
                /* handle that line */
                for (vector<mg_patt>::iterator pat = patterns.begin(); pat != patterns.end(); pat++) {
                    if (pat->match(tline)) { 
                        #pragma omp critical
                        finds.push_back(mg_find(tline, files[j], k));
                    }
                }
            }
            f.close();
        ...
        for (vector<string>::iterator fi = files.begin(); fi != files.end(); fi++) {
            ...
            i = 0;
            vector<string> lines;
            while (mygetline(&f, &line, &term) == 0) /* read in lines */
                lines.push_back(line);
            #pragma omp parallel for
            for(unsigned int j = 0; j < lines.size(); j++) { /* while able to read in line */
                i++; /* incr line counter */
                /* handle that line */
                for (vector<mg_patt>::iterator pat = patterns.begin(); pat != patterns.end(); pat++) {
                    if (pat->match(lines[j])) { 
                        #pragma omp critical
                        finds.push_back(mg_find(lines[j], *fi, i));
                    }
                }
            }
            f.close();
        }
    } else if (parall_pattern) {
        for (vector<string>::iterator fi = files.begin(); fi != files.end(); fi++) {
            ifstream f((*fi).c_str());
            if (!f.is_open())
                continue; /* nonexistant file */
